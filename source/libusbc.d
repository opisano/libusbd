 ...
 * Translated to D by Olivier Pisano <olivier.pisano@laposte.net>
...
private ushort libusb_cpu_to_le16(ushort x)
{
...
private ushort libusb_le16_to_cpu(ushort x)
{
    version (LittleEndian)
    {
        return x;
...
LIBUSB_DT_BOS_MAX_SIZE = LIBUSB_DT_BOS_SIZE + LIBUSB_BT_USB_2_0_EXTENSION_SIZE
...
enum LIBUSB_TRANSFER_TYPE_MASK = 0x03;    /* in bmAttributes */
...
enum LIBUSB_ISO_SYNC_TYPE_MASK = 0x0C;
...
enum LIBUSB_ISO_USAGE_TYPE_MASK = 0x30;
...
struct libusb_config_descriptor {
	...
	const(libusb_interface)* interface_;
...
enum LIBUSB_CONTROL_SETUP_SIZE = libusb_control_setup.sizeof;
 ...
 * notified about.
 */
alias void function(libusb_transfer *transfer) libusb_transfer_cb_fn;
...
struct libusb_transfer {
	...
	libusb_iso_packet_descriptor[1] iso_packet_desc;
...
libusb_device_handle *  libusb_open_device_with_vid_pid(
	libusb_context *ctx, ushort vendor_id, ushort product_id);
 ...
 */                              
ubyte* libusb_control_transfer_get_data(libusb_transfer *transfer)
{
 ...
 */
libusb_control_setup *libusb_control_transfer_get_setup(
 ...
 * \ref libusb_control_setup
 */
void libusb_fill_control_setup(ubyte* buffer,
	ubyte bmRequestType, ubyte bRequest, ushort wValue, ushort wIndex,
	ushort wLength)
{
	auto setup = cast(libusb_control_setup *)cast(void *) buffer;
...
void libusb_fill_control_transfer(
	...
	ubyte *buffer, libusb_transfer_cb_fn callback, void *user_data,
	uint timeout)
{
	auto setup = cast(libusb_control_setup *)cast(void *) buffer;
	...
	if (setup)
		transfer.length = cast(int) (LIBUSB_CONTROL_SETUP_SIZE
...
void libusb_fill_bulk_transfer(libusb_transfer *transfer,
	...
	ubyte *buffer, int length, libusb_transfer_cb_fn callback,
...
void libusb_fill_interrupt_transfer(
	libusb_transfer *transfer, libusb_device_handle *dev_handle,
	ubyte endpoint, ubyte *buffer, int length,
...
void libusb_fill_iso_transfer(libusb_transfer *transfer,
	libusb_device_handle *dev_handle, ubyte endpoint,
	ubyte *buffer, int length, int num_iso_packets,
	libusb_transfer_cb_fn callback, void *user_data, uint timeout)
{
	transfer.dev_handle = dev_handle;
	transfer.endpoint = endpoint;
...
void libusb_set_iso_packet_lengths(
	libusb_transfer *transfer, uint length)
{                   
	foreach (i; 0..transfer.num_iso_packets)
 ...
 */
ubyte *libusb_get_iso_packet_buffer(
 ...
 */
ubyte *libusb_get_iso_packet_buffer_simple(
	libusb_transfer *transfer, uint packet)
{
	int _packet;
...
int  libusb_control_transfer(libusb_device_handle *dev_handle,
	...
	ubyte *data, ushort wLength, uint timeout);

int  libusb_bulk_transfer(libusb_device_handle *dev_handle,
	ubyte endpoint, ubyte *data, int length,
 ...
 */
int libusb_get_descriptor(libusb_device_handle *dev,
	ubyte desc_type, ubyte desc_index, ubyte* data, int length)
{                                          
	return libusb_control_transfer(dev, libusb_endpoint_direction.LIBUSB_ENDPOINT_IN,
		libusb_standard_request.LIBUSB_REQUEST_GET_DESCRIPTOR, cast(ushort) ((desc_type << 8) | desc_index),
		0, data, cast(ushort) length, 1000);
}
...
int libusb_get_string_descriptor(libusb_device_handle *dev,
	ubyte desc_index, ushort langid, ubyte *data, int length)
{
	return libusb_control_transfer(dev, libusb_endpoint_direction.LIBUSB_ENDPOINT_IN,
		libusb_standard_request.LIBUSB_REQUEST_GET_DESCRIPTOR, cast(ushort)((libusb_descriptor_type.LIBUSB_DT_STRING << 8) | desc_index),
		langid, data, cast(ushort) length, 1000);
}

int  libusb_get_string_descriptor_ascii(libusb_device_handle *dev,
	ubyte desc_index, ubyte *data, int length);
 ...
 */                       
alias void function(int fd, short events, void* user_data) libusb_pollfd_added_cb;
 ...
 * \see libusb_set_pollfd_notifiers()
 */
alias void function(int fd, void* user_data) libusb_pollfd_removed_cb;

const(libusb_pollfd) **  libusb_get_pollfds(
	libusb_context *ctx);        
 ...
 */ 
alias int libusb_hotplug_callback_handle;
 ...
 */
alias int function(libusb_context *ctx,
						libusb_device *device,
						libusb_hotplug_event event,
						void *user_data) libusb_hotplug_callback_fn;  
